    ...
    <>
      ...
      <form className="mx-auto max-w-4xl rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7" onSubmit={(event) => { event.preventDefault(); if (canCreate) create.mutate(); }}>
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <label className="text-sm font-bold text-slate-800 sm:col-span-2">Select Service <span className="text-red-600">*</span><select className="mt-2 h-11 w-full rounded-md border border-slate-200 bg-white px-3 text-sm" value={form.required_service} onChange={(event) => setForm({ ...form, required_service: event.target.value })}><option value="">Search or select a service</option>{services.data?.results.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select></label>
        </div>
