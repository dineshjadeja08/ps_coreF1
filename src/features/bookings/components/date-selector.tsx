...
export function DateSelector({ selectedDate, onSelectDate }: { selectedDate: string; onSelectDate: (date: string) => void }) {
  ...
  return (
    <div className="mobile-scroll-row flex snap-x snap-proximity gap-2 overflow-x-auto pb-2 sm:grid sm:grid-cols-5">
      {dates.map((date) => (
        <button
          key={date.value}
          type="button"
          ...
          className={cn(
            "min-h-16 min-w-16 shrink-0 snap-start rounded-lg border px-2 py-2 text-center text-sm font-semibold transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary",
