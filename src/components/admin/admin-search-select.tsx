...
export function AdminSearchSelect({ value, options, placeholder, onChange, onSearch, loading = false }: {
  ...
  return (
    <div ref={container} className="relative">
      <button type="button" role="combobox" aria-controls={optionsId} aria-expanded={open} onClick={() => setOpen((current) => !current)} onBlur={(event) => { if (!container.current?.contains(event.relatedTarget)) setOpen(false); }} className="flex h-11 w-full items-center justify-between rounded-md border border-slate-200 bg-white px-3 text-left text-sm">
      ...
      {open ? <div className="absolute inset-x-0 top-full z-40 mt-1 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl">
        ...
        <div id={optionsId} className="max-h-60 overflow-y-auto p-1">
